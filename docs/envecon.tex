...
Also, people prefer to delay incurring costs (would you rather pay someone \$100 now or a year from now?). Conceivably, there is some offer at which you would be indifferent between the current payment (of \$100) and the future payment (of, say, \$110 or some other value that is greater than \$100). The present value of a future amount of net benefits is the amount which, if the person received it today, would make him just as well off as if he waited to receive those net benefits in the future. This indifference between two net benefits separated by time determines an individual's discount factor. Disount factor is a number indicating how much a person discounts (net) benefits that accrue in the future relative to the present.
...
\[NPV = \sum_{t=0}^{T}\delta^t\left(B_t-C_t\right),\]
where \(\beta^t\) is the discount factor associated with period \(t\).
